...
#define LED_PIN     1 // LEDstrip pin nummer
#define NUM_LEDS    20 // aantal leds wat je 
...
int FSR = A0; // druksensor
int glassesPerDay = 0; // variabele voor het aantal leeggedronken glazen
int glassesPerDayOld = 0;
int counter = 0; // variabele voor de functie om glazen te tellen
CRGB leds[NUM_LEDS];
//display variable
...
void setup() {
  ...
  Serial.println(testFillScreen());                                   //intro scherm met SMART MOASEPAD
...
void loop() {
  int FSRValue = analogRead(FSR); // waarde van de druksensor
  puntenTotaal = (puntenWater*0.5) + (puntenLopen*0.5);
  puntenGroep = (puntenTotaal*0.25) + (0.25) + (0.125); 
  puntenTotaalLed = ceil((puntenGroep*10) + 10);
  //voor het uit lezen van de button
  buttonState = digitalRead(buttonPin);
  if (buttonState == HIGH) {
    if (longHighTime.done()) {                  //Detecteeerd al;s de knop lang word ingedrukt
      Serial.println("HIGH-long");
      buttonTrigger=0;
      buttonTriggerHigh=1;
    }else {                                    //Als de knop niet lang word in gedrukt
      Serial.println("HIGH-short");
      buttonTrigger=1;
    }
  } else {
    if(buttonTrigger==1){                      //Hier word bepaald welke counter er om hoog mag dit verscheel per scherm vandaar die vele voorwaarde en variable.
      count++;
      buttonTrigger=0;
    }
  ...
  */
  if (count==2){                      //scherm 2
    if (buttontrigger2==0){
      tft.fillScreen(ILI9341_BLACK);
      testText("1/1",300,0, ILI9341_WHITE,1);
      PersentageBar(ILI9341_YELLOW, puntenLopen, 40, "Lopen"); //perentage toevoegen
  ...
  if (count==4){                    //scherm 3 eigen groep informatie
      tft.fillScreen(ILI9341_BLACK);
      testText("1/1",300,0, ILI9341_WHITE,1);
  ...
  if (count==6){                    //scherm 4 groep informatie
      tft.fillScreen(ILI9341_BLACK);
      testText("1/1",300,0, ILI9341_WHITE,1);
  ...
  if (count==8){                  //scherm 5 led indicatie scherm 2 opties mogelijk
  ...
  if (count>9){                                 //loop de screen counter
    count=0;
  }
  ...
  if(FSRValue >300 && FSRValue < 580 && counter ==1) { 
    glassesPerDay++;
    counter = 0;
  } //Nog toevoegen: while 10 secondenlang deze waarde
  // Vol glas:
  else if (FSRValue > 580) {
    counter = 1;
  }
  // LEDs voor 0 glazen, 10 leds rood
  if(glassesPerDay == 0){
      for (int i = 0; i <= 19; i++) {
      leds[i] = CRGB ( 255, 0, 0);
      FastLED.show();
  ...
  //  LEDs voor 1 glas, 2 leds groen en 1 led half
  if(glassesPerDay == 1){
      for (int i = 0; i <= 1; i++) {
        leds[i] = CRGB ( 0, 255, 0);
        FastLED.show();
  ...
  // LEDs voor 2 glazen, 5 leds groen
  if(glassesPerDay == 2){
        for (int i = 0; i <= 4; i++) {
        leds[i] = CRGB ( 0, 255, 0);
        FastLED.show();
  ...
  // LEDs voor 3 glazen, 7 leds groen en 1 led half
  if(glassesPerDay == 3){
      for (int i = 0; i <= 6; i++) {
        leds[i] = CRGB ( 0, 255, 0);
        FastLED.show();
  ...
  // LEDs voor 4 glazen, 10 leds groen
  if(glassesPerDay == 4){
      for (int i = 0; i <= 9; i++) {
        leds[i] = CRGB ( 0, 255, 0);
        FastLED.show();
  ...
  if(puntenTotaalLed == 11) {
    ...
    if (glassesPerDay!=glassesPerDayOld){
    count--;
    glassesPerDayOld=glassesPerDay;
  }
//  // LEDS voor groepspercentage
//  for (int i = 10; i <=20; i++) {
//    if(puntenTotaalLed = i){
//      leds[i] = CRGB (102,102,255);
//        FastLED.show();
//        //delay(40);
//    }
...
unsigned long testText(char Str1[], int positiex, int positiey, uint16_t color1, int TextSize ) {   //voor het invoeren van text
  //tft.fillScreen(ILI9341_BLACK);
...
unsigned long testText2(int chard, int positiex, int positiey, uint16_t color1, int TextSize ) { //voor het invoeren van text met een variable cijfer
  //tft.fillScreen(ILI9341_BLACK);
  unsigned long start = micros();
  tft.setCursor(positiex, positiey);    //SHIFT,REGEL
  tft.setTextColor(color1);  tft.setTextSize(1);
...
unsigned long testFillScreen() {              //opstart scherm
  unsigned long start = micros();
  tft.fillScreen(ILI9341_BLACK);
  tft.setRotation(3);
...
unsigned long testFastLines(uint16_t color1, uint16_t color2) {  //in vullen van 2 gekleurede gedeeltes zoals scherm 5 opties 1 en 2
  unsigned long start;
...
unsigned long blackLines() { //voor hat zwart maken van kleinre gedeelte
  unsigned long start;
  int           x, y, w = tft.width(), h = tft.height();
  start = micros();
...
unsigned long blackLines2() { //voor hat zwart maken van kleinre gedeelte
  unsigned long start;
  int           x, y, w = tft.width(), h = tft.height();
  start = micros();
...
unsigned long PersentageBar(uint16_t color1, float procentige, int positionh, char Str1[] ) { //voor de persentage balken met varbable input.
  unsigned long start;
  int           x, y;
